 ...
 */
class TwitterOAuth extends TwitterConfig
{
    ...
    public function __construct($consumerKey, $consumerSecret, $oauthToken = null, $oauthTokenSecret = null)
    {
        ...
        $this->signatureMethod = new TwitterHmacSha1();
        $this->consumer = new TwitterConsumer($consumerKey, $consumerSecret);
        if (!empty($oauthToken) && !empty($oauthTokenSecret)) {
            $this->token = new TwitterToken($oauthToken, $oauthTokenSecret);
        }
        if (empty($oauthToken) && !empty($oauthTokenSecret)) {
    ...
    public function oauth2($path, array $parameters = array())
    {
        ...
        $this->response->setApiPath($path);
        $url = sprintf('%s/%s', self::API_HOST, $path);
        $request = TwitterRequest::fromConsumerAndToken($this->consumer, $this->token, $method, $url, $parameters);
        ...
        $response = TwitterJsonDecoder::decode($result, $this->decodeJsonAsArray);
        $this->response->setBody($response);
        return $response;
    }
    ...
    private function request($url, $method, $headers, $postfields)
    {
        ...
        switch ($method) {
            case 'GET':
                if (!empty($postfields)) {
                    $options[CURLOPT_URL] .= '?' . TwitterUtil::buildHttpQuery($postfields);
                }
                break;
            case 'POST':
                $options[CURLOPT_POST] = true;
                $options[CURLOPT_POSTFIELDS] = TwitterUtil::buildHttpQuery($postfields);
                break;
        }
