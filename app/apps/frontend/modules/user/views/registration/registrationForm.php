...
<div>
    ...
    <div class="row control-group <?php echo $model->hasErrors('password_repeat') ? 'error' : '' ?>">
        ...
        <div class="input">
            <?php echo $form->PasswordField($model, 'password_repeat', array('class'=>'text', 'placeholder' => 'Введите пароль')); ?>
            <?php if($model->hasErrors('password_repeat')): ?><span class="help-inline"><?php echo $model->getError('password_repeat') ?></span><?php endif ?>
        </div>
    </div>

    <?php $captcha = $this->beginWidget('WCaptcha', array('model'=>$model, 'attribute'=>'captcha', 'captchaAction'=>'captcha_registration')); ?>
    ...
    <div class="row control-group <?php echo $model->hasErrors('accept_process') ? 'error' : '' ?>">
        <div class="input">
            <label><?php echo $form->Checkbox($model, 'accept_process', array('class' => 'check')); ?> Я согласен с <a href="/rules" target="_blank">правилами</a>
            <?php if($model->hasErrors('accept_process')): ?><span class="help-inline"><?php echo $model->getError('accept_process') ?></span><?php endif ?>
        </div>
    </div>
